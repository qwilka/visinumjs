...
export class VnNode {
    ...
    // console.log("class VnNode this", this)
    // console.log("class VnNode data", data)
    // for (let key in data) {
    //     if (data.hasOwnProperty(key)) console.log("key=",key, "data[key]=", data[key])
    // }
    // console.log("class VnNode data.childs", data["childs"])
    // console.log("class VnNode this.name", this.name)
    // console.log("class VnNode this.vn_type", this.vn_type)
    //  console.log("class VnNode this.childs", this.childs)
    }

    merge_nodeObj(nodeObj) {
        for (let key in nodeObj) {
            if ( _.indexOf(["parent", "childs"], key) >= 0) continue
            this[key] = nodeObj[key]
        }
        //if ("childs" in nodeObj) {    // not safe
        if (nodeObj.hasOwnProperty("childs")) {
            for (let childObj of nodeObj.childs) {
                new VnNode(null, this, childObj) // parent.add_child in constructor
            }
        }
    }
    ...
    *traverse() {
    ...
    toAnlzTree() {
        ...
        return new VnNode(null, null, _anlzObj)
    }
...
function runTests() {
    ...
    let newnode = new VnNode("", null, rootnode.toTreeObj()) // create new tree using Obj from first tree
